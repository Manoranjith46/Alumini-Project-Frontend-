...
export interface TokenAuthContextType {
  ...
  loading: boolean;
  loginWithToken: (tokenData: any) => TokenSession;
  logoutToken: () => void;
  markTokenUsed: () => void;
...
export default function TokenAuthProvider({ children }: TokenAuthProviderProps) {
  ...
  useEffect(() => {
    ...
    if (storedSession) {
      try {
        const parsed = JSON.parse(storedSession) as TokenSession;
  ...
  const loginWithToken = (tokenData: any): TokenSession => {
